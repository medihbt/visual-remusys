...
function handleEditorMount(
  ...
  monacoRef: React.RefObject<Monaco>,
) {
  try {
    const exists = monaco.languages
      .getLanguages()
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      .some((l: any) => l.id === "llvm");
...
export default function LensViewer({ irText, srcType }: LensViewerProps) {
  ...
  const moduleOverview = useIRStore(
    (s) => s.module?.brief.overview_src ?? s.sourceText,
  );
  ...
  const language =
    srcType === "ir" ? "llvm" : srcType === "sysy" ? "c" : "text";
  ...
  return (
    <Editor
      ...
      onMount={(editor, monaco) =>
        handleEditorMount(editor, monaco, editorRef, monacoRef)
      }
      options={{
        readOnly: true,
