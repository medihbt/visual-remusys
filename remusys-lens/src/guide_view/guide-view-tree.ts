...
function connectGuideTree(node: GuideNodeExpand) {
  for (const child of node.children) {
    child.parent = node;
    if (isGuideNodeExpand(child)) {
      connectGuideTree(child);
    }
  }
}

export function isGuideNodeExpand(
  node: GuideNodeData,
): node is GuideNodeExpand {
...
function ensureExpandTree(
  irStore: IRState,
  state: GuideTreeController,
): { moduleId: number; expandTree: IRExpandTree } {
  const module = irStore.getModule();
...
export function refreshSameModule(
  controller: GuideTreeController,
  irStore: IRState,
): GuideTreeBuildResult {
...
export function resetForNewModule(
  controller: GuideTreeController,
  irStore: IRState,
): GuideTreeBuildResult {
  const module = irStore.getModule();
...
/** 展开这个结点和它的一层子结点（占位）。 */
export function expandChildrenNode(
  controller: GuideTreeController,
  irStore: IRState,
  node: GuideNodeData,
): GuideTreeBuildResult {
...
export function collapseChildrenNode(
  controller: GuideTreeController,
  irStore: IRState,
  node: GuideNodeData,
): GuideTreeBuildResult {
...
/** 通过节点请求焦点路径（路径由当前树重建）。 */
export function requestFocusNode(
  controller: GuideTreeController,
  irStore: IRState,
  node: GuideNodeData,
): GuideTreeBuildResult {
...
/** 请求设置焦点路径，并基于新焦点重建树。 */
export function requestFocusPath(
  controller: GuideTreeController,
  irStore: IRState,
  requestedPath: IRObjPath,
): GuideTreeBuildResult {
  const { moduleId, expandTree } = ensureExpandTree(irStore, controller);
  const nextFocusPath = ensureNonEmptyPath(
    sliceValidObjPath(irStore.getModule(), requestedPath),
  );
 ...
 */
export function loadExpandedGuideTree(
  controller: GuideTreeController,
  irStore: IRState,
): GuideNodeExpand {
