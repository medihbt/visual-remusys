...
function makeGroupNodes(sections: SectionModel[], focus: IRObjPath): FlowGroupNode[] {
    return sections.map((section) => {
        const palette = rolePalette(section.kind);
        const focused = section.nodes.some((node) => focusMatchesIRObject(focus, dfgNodeToIRObjID(node)));
        return {
            id: section.id,
            type: "groupNode",
            position: { x: 0, y: 0 },
            width: SECTION_WIDTH,
            height: SECTION_HEADER_HEIGHT + SECTION_PADDING * 2 + Math.max(section.nodes.length, 1) * (NODE_HEIGHT + 12),
            data: {
...
function makeElemNodes(sections: SectionModel[], focus: IRObjPath): FlowElemNode[] {
...
function buildElkGraph(sections: SectionModel[], nodes: FlowElemNode[], edges: FlowEdge[]): ElkNode {
    ...
    return {
        ...
        children: sections.map((section) => ({
            id: section.id,
            width: SECTION_WIDTH,
            height: SECTION_HEADER_HEIGHT + SECTION_PADDING * 2 + sectionChildHeight({ kind: section.kind, nodes: section.nodes }),
            layoutOptions: {
                "elk.algorithm": "layered",
                "elk.direction": "DOWN",
...
function edgePathFromSections(sections: ElkEdgeSection[]): { path: string; labelPosition: { x: number; y: number } } {
...
function resolveChildNodePosition(elkNode: ElkNode, parent: ElkNode | null): { x: number; y: number } {
...
function routeSectionOrderEdge(edge: FlowEdge, groupNodeMap: Map<string, FlowGroupNode>): void {
    ...
    const sourceHeight = source.height ?? (SECTION_HEADER_HEIGHT + SECTION_PADDING * 2 + NODE_HEIGHT);
    const targetHeight = target.height ?? (SECTION_HEADER_HEIGHT + SECTION_PADDING * 2 + NODE_HEIGHT);
...
async function layoutBlockDfgWithOrderEdges(
    ...
    const elemNodes = nodes.filter((node): node is FlowElemNode => node.type === "elemNode");
    const groupNodes = nodes.filter((node): node is FlowGroupNode => node.type === "groupNode");
...
export async function getBlockDfg(irState: IRState, blockID: BlockID): Promise<FlowGraph> {
    const blockDfg = irState.getBlockDfg(blockID);
    ...
    return layoutBlockDfgWithOrderEdges([...groupNodes, ...elemNodes], dataEdges, orderEdges, sections);
}